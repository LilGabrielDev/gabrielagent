...
The Admin page provides user account management and API key administration for your Gabriel instance. This page is accessible from the sidebar under the "Admin" section.
...
1. **Users** -- Manage admin accounts that can access the Gabriel dashboard.
...
Gabriel supports three roles with different access levels:
...
- **Admin**: Complete control over the Gabriel instance. Can manage all settings, users, knowledge base, conversations, tickets, channels, and integrations. At least one admin account must exist at all times.
...
Gabriel prevents deleting the last remaining admin account. If you attempt to delete a user and they are the only account with the `admin` role, the operation will be rejected with an error message. This safeguard ensures you cannot lock yourself out of the system.
...
- Authentication uses JWT tokens stored in HTTP-only cookies (`gabriel-token`).
